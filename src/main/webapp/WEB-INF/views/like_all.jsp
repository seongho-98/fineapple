...
<body>
	...
	찜 번호: ${{vo.like_id}} <br> <!-- el: expression language, 속성으로 지정한 것만 프린트! -->
	상품 이름: ${{vo.product_name}} <br>
	상품 가격: ${{vo.product_price}} <br>
	카테고리 번호: ${{vo.product_category}} <br>
	판매자 ID: ${{vo.sellor_id}} <br>
	거래 상태: ${{vo.product_status}} <br>
	<hr color="red">
	</c:forEach>
</body>
</html>